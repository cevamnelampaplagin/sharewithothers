(function() {
    ...
    function startPlugin() {
        ...
        function addHeaderButton() {
            ...
            var svgIcon = '<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M9 3L7.17 5H4C2.9 5 2 5.9 2 7V19C2 20.1 2.9 21 4 21H20C21.1 21 22 20.1 22 19V7C22 5.9 21.1 5 20 5H16.83L15 3H9Z" fill="none" opacity="0.3"/><path d="M20 5H16.83L15 3H9L7.17 5H4C2.9 5 2 5.9 2 7V19C2 20.1 2.9 21 4 21H20C21.1 21 22 20.1 22 19V7C22 5.9 21.1 5 20 5ZM12 18C9.24 18 7 15.76 7 13C7 10.24 9.24 8 12 8C14.76 8 17 10.24 17 13C17 15.76 14.76 18 12 18Z" fill="currentColor"/><circle cx="12" cy="13" r="3" fill="currentColor"/></svg>';
            var button = $('<div class="head__action open--photo-search selector" title="' + Lampa.Lang.translate('photo_search_button') + '">' + svgIcon + '</div>');
        ...
        function openPhotoSearchWindow() {
            ...
            var html = $(
                ...
                '<div id="ps-inner" style="display:flex;flex-direction:column;align-items:center;gap:8px;color:rgba(255,255,255,.55);font-size:24px;font-weight:500;text-align:center;padding:0 16px;pointer-events:none;">' +
                ...
                '<input type="text" id="ps-url-input" class="selector" placeholder="https://..." style="width:100%;padding:10px;border-radius:6px;border:1px solid rgba(255,255,255,0.2);background:rgba(0,0,0,0.3);color:#fff;outline:none;font-size:14px;box-sizing:border-box;">' +
                '</div>' +
            ...
            setTimeout(function() {
                ...
                if (fileInput) {
                    fileInput.addEventListener('change', function() {
                        ...
                        reader.onload = function(e) {
                            $('#ps-inner').html('<img src="' + e.target.result + '" style="max-width:100%;max-height:165px;border-radius:8px;">');
                        };
                ...
                if (urlInput) {
                    urlInput.addEventListener('input', function() {
                        ...
                        $('#ps-inner').html('<img src="' + url + '" style="max-width:100%;max-height:165px;border-radius:8px;" onerror="this.style.display=\'none\'">');
                    });
                }
        ...
        function sendImageToIdentifier() {
            ...
            function performRequest(fileBlob) {
                ...
                .then(data => {
                    ...
                    if (!result || !result.title && !result.name) throw new Error('Not found');
                    
                    $('#ps-loader-text').text(Lampa.Lang.translate('photo_search_searching_tmdb'));
                    searchTmdb(result.name || result.title, result);
                })
                .catch(err => {
                    hideLoader();
            ...
            if (urlVal && !selectedFile) {
                ...
                var proxyUrl = 'https://corsproxy.io/?' + encodeURIComponent(urlVal);
                ...
                fetch(proxyUrl)
                    ...
                    .catch(() => {
                        hideLoader();
                        Lampa.Noty.show(Lampa.Lang.translate('photo_search_network_error') + ' (CORS/URL)');
                    });
            } else {
        ...
        function searchTmdb(title, info) {
            var apiKey = getTmdbApiKey();
            var lang = getTmdbLang();
            var url = 'https://api.themoviedb.org/3/search/multi?api_key=' + apiKey + '&query=' + encodeURIComponent(title) + '&language=' + lang;
