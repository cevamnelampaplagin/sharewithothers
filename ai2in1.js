(function () {
    ...
    var NativeRandomSource = {
        list: function(params, oncomplite, onerror) {
            ...
            // Формування запиту з розширеною логікою для аніме
            function getEndpointAndQuery() {
                var endpoint, query = [];
                
                if (isAnime) {
                    // 70% серіали, 30% фільми для аніме
                    endpoint = Math.random() < 0.7 ? 'tv' : 'movie';
                    query.push('with_genres=16'); 
                    query.push('with_keywords=210024|287501'); 
                } else {
                    if (type === 'movie') { 
                        endpoint = 'movie'; 
                ...
                return { endpoint: endpoint, baseQuery: "&" + query.join('&') };
            }

            var maxPage = 100;
            if (isAnime) maxPage = 20; 
            ...
            function fetchBatch() {
                ...
                Lampa.Network.silent(Lampa.TMDB.api(url), function(data) {
                    // Виправлення багу з пустими сторінками з V50
                    if (data && data.total_pages) {
                        var actualMaxPage = Math.min(data.total_pages, 500);
                        if (actualMaxPage < maxPage) {
                            maxPage = actualMaxPage;
                            if (randomPage > maxPage) return fetchBatch();
                        }
                    }

                    if (data && data.results) {
                        data.results.forEach(function(item) {
                            if (type === 'cartoon' && item.original_language === 'ja') return; // Виключаємо аніме з мультиків
                            if (isAnime && item.genre_ids.indexOf(16) === -1) return; // Строга перевірка аніме
    ...
    function parseJson(text) { 
        try { 
            // Покращене очищення розмітки з V50
        ...
        discovery: function () {
            return {
                ...
                search: function (params, done) {
                    ...
                    fetch('[https://generativelanguage.googleapis.com/v1beta/models/](https://generativelanguage.googleapis.com/v1beta/models/)' + TARGET_MODEL + ':generateContent?key=' + key, { 
                        ...
                        function next() {
                            ...
                            // Збільшено до 5 паралельних запитів з V51
                            if (!queue.length || active >= 5) return; 
    ...
    function startPlugin() {
        ...
        addButtons();
        console.log('AI Search: V52 (Merged: Ratings, Age Toggle, Anime Fix, Paging Fix, Markdown parser)');
    }
