// @name: Backmenu
// @version: 2
// @description: Меню за кнопкою назад (виправлено)
  ...
  (function () {
    ...
    function vel() {
      ...
      function loralynn() {
        ...
        // Іконки SVG (скорочено для читабельності)
        ...
        Lampa.Select.show({
            title: "Вихід", 
            items: gadi, 
            // Виправлено: замість Lampa.Controller.toggle використовуємо Lampa.Select.hide()
      ...
      // Перехоплення вибору пункту "Вийти" в головному меню
      Lampa.Controller.listener.follow("toggle", function (loreli) {
        if (loreli.name == "select" && $(".selectbox__title").text() == Lampa.Lang.translate("title_out")) {
          Lampa.Select.hide();
          // Виправлено: прибрано setTimeout, щоб уникнути повторного відкриття
          loralynn();
        }
      });
    }
