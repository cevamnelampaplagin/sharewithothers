// @name: Backmenu
// @version: 2.1
// @description: Меню за кнопкою назад (стабільна версія)
(function () {
  "use strict";
  ...
  (function () {
    ...
    function vel() {
      var backMenuVisibilityValues = {1: "Приховати", 2: "Показати"};
      var backMenuItems = [
        {name: "exit",          defaultValue: "2", title: "Закрити додаток/сайт"},
        {name: "reboot",        defaultValue: "2", title: "Перезавантажити"},
        {name: "switch_server", defaultValue: "2", title: "Замінити сервер"},
        {name: "clear_cache",   defaultValue: "2", title: "Очистити кеш"},
      ...
      function nitza() {
        ...
        // HTTPS-версія спідтесту (без змішаного вмісту)
      ...
      // Прапорець для запобігання повторному виклику меню
      var backMenuLock = false;

      function loralynn() {
        if (backMenuLock) return;
        backMenuLock = true;

        var gadi = [];
        
        // Іконки SVG (скорочено)
        ...
        if (localStorage.getItem("exit") !== "1")          gadi.push({title: makeItem(icon_exit, "Закрити додаток/сайт"), action: "exit"});
        ...
        try {
          Lampa.Select.show({
            ...
            onBack: function() {
              Lampa.Select.hide();
              backMenuLock = false; // дозволяємо наступне відкриття
            },
          ...
          console.warn("Backmenu: помилка відкриття меню", e);
          backMenuLock = false; // скидаємо блокування у разі помилки
        }
      }

      // Перехоплення вибору "Вийти" в головному меню
      Lampa.Controller.listener.follow("toggle", function (loreli) {
        if (loreli.name == "select" && $(".selectbox__title").text() == Lampa.Lang.translate("title_out")) {
          Lampa.Select.hide();
          // Невелика затримка, щоб уникнути конфлікту з history.pushState
          setTimeout(function() {
            loralynn();
          }, 50);
        }
