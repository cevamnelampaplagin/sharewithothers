// @name: Backmenu
// @version: 1
// @description: Меню за кнопкою назад
// https://spxload.github.io/pl/Cubox/menus.js
(function () {
  "use strict";

  // Ініціалізація платформи TV (залишаємо як є)
  if(Lampa.Platform && Lampa.Platform.tv) Lampa.Platform.tv();

  (function () {
    // Внутрішня функція-обгортка (seiry) - залишаємо, вона не заважає
    var seiry = function () {
      var tersa = true;
    ...
    function vel() {
      // --- Видалена перевірка bylampa ---
      // if (Lampa.Manifest.origin !== "bylampa") { ... }

      // Конфіг для пунктів меню виходу
      ...
      // Пункт «Меню Вихід» у розділі «Інше»
      Lampa.SettingsApi.addParam({
        component: "more",
        param: {name: "back_menu", type: "static", default: true},
        field: {
          name: "Меню Вихід",
          description: "Налаштування відображення пунктів меню"
        },
      ...
      backMenuItems.forEach(function (item) {
        Lampa.SettingsApi.addParam({
          ...
          field: {
            name: item.title,
            description: "Натисніть, щоб вибрати"
          }
        });
      });

      // Ініціалізація значень за умовчанням
      ...
      function aubreanna() {
        Lampa.Storage.clear();
        // Перезавантаження після очищення кешу для надійності
      ...
      function lakeeta() {
        Lampa.Input.edit({title: "Вкажіть сервер", value: "", free: true}, function (tiquita) {
          if (tiquita !== "") {
      ...
      function season() {
        // Додав перевірку на webOS для старих телевізорів
      ...
      // Функція відображення меню (залишив так, тільки іконки SVG довгі)
      function loralynn() {
        var gadi = [];
        
        // --- ICONS (SVG) ---
        // Я скоротив код SVG для читання, але функціонал той же
        ...
        // Helper для створення HTML пункта
        ...
        if (localStorage.getItem("exit") !== "1")          gadi.push({title: makeItem(icon_exit, "Закрити програму/сайт"), action: "exit"});
        if (localStorage.getItem("reboot") !== "1")        gadi.push({title: makeItem(icon_reboot, "Перезавантажити"), action: "reboot"});
        if (localStorage.getItem("switch_server") !== "1") gadi.push({title: makeItem(icon_server, "Змінити сервер"), action: "server"});
        if (localStorage.getItem("clear_cache") !== "1")   gadi.push({title: makeItem(icon_cache, "Очистити кеш"), action: "cache"});
        
        // Зовнішні програми (залишив стандартну іконку icon_app для простоти)
        ...
        Lampa.Select.show({
            ...
            onSelect: function(item) {
                // Використовуємо поле 'action' замість порівняння довгих HTML рядків (це надійніше)
                switch(item.action) {
      ...
      // Перехоплення кнопки виходу в головному меню
